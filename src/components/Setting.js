  ...
  onClickSave = () => {
    ...
    let new_state = this.userSubscribeCoin.map(coin => {
      ...
      coin.ASK.support_market.forEach(market => {
        toSetCoinSubcribe.ASK.sub_status.push(switch_status[`ASK_${coin.name}_${market}`]);
      });

      coin.BID.support_market.forEach(market => {
        toSetCoinSubcribe.BID.sub_status.push(switch_status[`BID_${coin.name}_${market}`]);
      });
...
class RenderCoinSettingCard extends Component {
  ...
  handleChange = (checked, event, id) => {
    ...
    this.state.switch_status[`${type}_${coin_name}_${market}`] = !this.state.switch_status[`${type}_${coin_name}_${market}`];
          ...
          <div className="settingCard darkgray" key = {index}>
              ...
              coin.ASK.support_market.map(market => {
                return (
                  <div className="toggleText"> {market} 
                    <Switch onChange={this.handleChange} checked = {this.state.switch_status[`ASK_${coin.name}_${market}`]} className="toggleButton" id={`ASK-${market}-${coin.name}-switch`} height={20} width={40} onColor="#1e90ff"/>
                  </div>
                )
              })
            }
            ...
            <div className="settingCard_coin">BID</div>
            {
              coin.BID.support_market.map(market => {
                return (
                  <div className="toggleText"> {market} 
                    <Switch onChange={this.handleChange} checked = {this.state.switch_status[`BID_${coin.name}_${market}`]} className="toggleButton" id={`BID-${market}-${coin.name}-switch`} height={20} width={40} onColor="#e55039"/>
                  </div>
                )
              })
            }
